...
| 改进 | 方案 | 特点 |
|---|---|---|
| 1 | Vicuna-7B → LLaMA-3.1-8B | 通用能力提升，改动最小，首选 |
| 5 | Vicuna-7B → Phi-3.5-mini | 参数减半，速度 2×，资源受限首选 |
| 7 | Vicuna-7B → Qwen2.5-7B | 中文能力极强，需额外适配工作 |
...
1. **首要：选定模块 A（语言模型基座）**
   改进 1 / 5 / 7 三选一，是所有后续训练的起点。一旦开始训练不可中途切换，必须在动手前根据目标场景（通用性能 / 部署速度 / 中文指令）确定。
...
第二批（低成本，1-2 天内出结果）
  ├─ 模块 A：选定改进 1 / 5 / 7 之一，启动训练（占 GPU，后台跑）
